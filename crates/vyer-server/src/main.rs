...
const SCRY_BLOCK_BEGIN: &str =
    "<!-- BEGIN SCRY MANAGED BLOCK (auto-generated by `vyer init`; edits between these markers are overwritten) -->";
const SCRY_BLOCK_END: &str = "<!-- END SCRY MANAGED BLOCK -->";
...
fn upsert_managed_block(existing: &str, block: &str) -> String {
    if let (Some(b), Some(e)) = (
        existing.find(SCRY_BLOCK_BEGIN),
        existing.find(SCRY_BLOCK_END),
    ) {
        if b < e {
            let end_idx = e + SCRY_BLOCK_END.len();
...
fn cmd_init(args: &[String]) -> Result<(), String> {
    ...
    let verb = if existing.is_empty() {
        "create"
    } else if existing.contains(SCRY_BLOCK_BEGIN) {
...
mod tests {
    use super::{
        positional, upsert_managed_block, vyer_managed_block, SCRY_BLOCK_BEGIN, SCRY_BLOCK_END,
    };
    ...
    fn upsert_managed_block_is_idempotent_and_surgical() {
        ...
        assert!(created.contains(SCRY_BLOCK_BEGIN) && created.contains(SCRY_BLOCK_END));
        ...
        assert!(appended.contains(SCRY_BLOCK_BEGIN));
        ...
        assert_eq!(
            twice.matches(SCRY_BLOCK_BEGIN).count(),
            1,
            "no duplicate block"
        );
        ...
        assert_eq!(refreshed.matches(SCRY_BLOCK_BEGIN).count(), 1);
    }
}
